...
export async function POST(request: NextRequest) {
  try {
    ...
    // Build the EXACT request body with CORRECT column names from API documentation
    const requestBody = {
      ...
      valueColumns: [
        ...
        { column: "buyer" },
        { column: "targetName" },
        { column: "callLengthInSeconds" }, // ✅ CORRECT: Based on ringba-api-samples
        { column: "connectedCallLengthInSeconds" }, // ✅ CORRECT: For connected duration
        { column: "timeToConnectInSeconds" }, // ✅ CORRECT: Time to connect
        { column: "inboundCallId" }, // ✅ CORRECT: Call ID
        { column: "hasConnected" }, // ✅ CORRECT: Connection status
        { column: "hasConverted" }, // ✅ CORRECT: Conversion status
        { column: "hasRecording" }, // ✅ CORRECT: Recording availability
        { column: "recordingUrl" }, // ✅ CORRECT: Recording URL
        { column: "conversionAmount" }, // ✅ CORRECT: Revenue
        { column: "payoutAmount" }, // ✅ CORRECT: Payout
        { column: "totalCost" }, // ✅ CORRECT: Cost
        { column: "endCallSource" }, // ✅ CORRECT: End call source
        { column: "targetNumber" }, // ✅ CORRECT: Target number
        { column: "number" }, // ✅ CORRECT: Tracking number
      ],
    }
    ...
    console.log("📊 Request body with CORRECT column names:", JSON.stringify(requestBody, null, 2))
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({
        ...
        requestBody,
        troubleshooting: {
          status400: "Check request body format - ensure column names match API documentation",
          status401: "Verify RINGBA_API_KEY is correct and using 'Token' prefix",
          status403: "Ensure API key has call logs permissions",
          status404: "Check the URL format and account ID",
          status422: "Check column names - using callLengthInSeconds (not callLength)",
          checkCampaignId: `Verify campaign ID '${campaignId}' exists in your RingBA account`,
          checkColumnNames: "Using correct column names from ringba-api-samples documentation",
        },
      })
    }
    ...
    // Transform the response using CORRECT field names
    ...
    if (data.isSuccessful && data.report && Array.isArray(data.report.records)) {
      transformedCallLogs = data.report.records.map((record: any) => ({
        ...
        trackingNumber: record.number,
        targetNumber: record.targetNumber,
        startTime: new Date(record.callDt).toISOString(),
        duration: Number.parseInt(record.callLengthInSeconds?.toString() || "0"), // ✅ Using correct field
        ...
        agentName: record.targetName || record.buyer,
    ...
    return NextResponse.json({
      ...
      transactionId: data.transactionId,
      campaignId,
      requestFormat: "CORRECTED_COLUMN_NAMES",
      columnNamesUsed: requestBody.valueColumns.map((col) => col.column),
